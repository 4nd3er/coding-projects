  ...
  return (
    ...
    >
      <Card className="bg-gradient-to-r from-cyan-500 to-blue-500 overflow-hidden">
        <img src={data?.image} className="w-full h-auto aspect-video object-cover" alt="" />
        <div
          className={`text-white z-20 absolute bottom-3 left-4 transition font-semibold ${data?.id === id
            ? "translate-y-0"
        ...
        <div className={`absolute inset-0 w-full h-full rounded-md ${data?.id === id
          ? "bg-gradient_h"
          : "bg-gradient"
          }`}></div>
      </Card>
